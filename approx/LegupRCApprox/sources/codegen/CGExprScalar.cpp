...
public:
  ...
  Value *EmitMul(const BinOpInfo &Ops) {
    ...
    if (Ops.Ty->isSignedIntegerOrEnumerationType()) {
      ...
      case LangOptions::SOB_Defined:
        value = Builder.CreateMul(Ops.LHS, Ops.RHS, "mul");
        inst = dyn_cast<llvm::Instruction>(value);
        if(inst){ 
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
        }
        return value;
      case LangOptions::SOB_Undefined:
        if (!CGF.SanOpts->SignedIntegerOverflow){
          value = Builder.CreateNSWMul(Ops.LHS, Ops.RHS, "mul");
          inst = dyn_cast<llvm::Instruction>(value);
          if(inst){ 
              inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
          }
      ...
      case LangOptions::SOB_Trapping:
        value = EmitOverflowCheckedBinOp(Ops);
        inst = dyn_cast<llvm::Instruction>(value);
        if(inst){ 
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
        }
    ...
    if (Ops.Ty->isUnsignedIntegerType() && CGF.SanOpts->UnsignedIntegerOverflow){
       ...
       inst = dyn_cast<llvm::Instruction>(value);
       if(inst){ 
           inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
       }
       return value;
    }
    
    if (Ops.LHS->getType()->isFPOrFPVectorTy()){
      value = Builder.CreateFMul(Ops.LHS, Ops.RHS, "mul");
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
          inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
      }
      return value; 
    }
  ...
  Value *EmitAnd(const BinOpInfo &Ops) {
    ...
    if(Ops.Ty->hasUnsignedIntegerRepresentation()){
      value = Builder.CreateAnd(Ops.LHS, Ops.RHS, "and");
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
      }
  ...
  Value *EmitXor(const BinOpInfo &Ops) {
     ...
     llvm::Instruction* inst;
     if(Ops.Ty->hasUnsignedIntegerRepresentation()){
      value = Builder.CreateXor(Ops.LHS, Ops.RHS, "xor");
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
      }
  ...
  Value *EmitOr (const BinOpInfo &Ops) {
    ...
    llvm::Instruction* inst;
    if(Ops.Ty->hasUnsignedIntegerRepresentation()){
      value = Builder.CreateOr(Ops.LHS, Ops.RHS, "or");
      inst = dyn_cast<llvm::Instruction>(value); 
      if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
      }
...
void ScalarExprEmitter::EmitFloatConversionCheck(Value *OrigSrc,
  ...
  if (llvm::IntegerType *IntTy = dyn_cast<llvm::IntegerType>(SrcTy)) {
    ...
    if (SrcIsUnsigned)
      ...
      //inst = dyn_cast<llvm::Instruction>(Check);
...
EmitComplexToScalarConversion(CodeGenFunction::ComplexPairTy Src,
  ...
  if (DstTy->isBooleanType()) {
    ...
    value = Builder.CreateOr(Src.first, Src.second, "tobool");
   // inst = dyn_cast<llvm::Instruction>(value);
...
Value *ScalarExprEmitter::VisitShuffleVectorExpr(ShuffleVectorExpr *E) {
  ...
  if (E->getNumSubExprs() == 2 ||
   ...
   // inst = dyn_cast<llvm::Instruction>(Mask);
    // newv = undef
    // mask = mask & maskbits
    // for each elt
    //   n = extract mask i
...
EmitAddConsiderOverflowBehavior(const UnaryOperator *E,
  ...
  switch (CGF.getLangOpts().getSignedOverflowBehavior()) {
  case LangOptions::SOB_Defined:
    value = Builder.CreateAdd(InVal, NextVal, IsInc ? "inc" : "dec");
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){
       inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
    }
  ...
  case LangOptions::SOB_Undefined:
    if (!CGF.SanOpts->SignedIntegerOverflow){
       value = Builder.CreateNSWAdd(InVal, NextVal, IsInc ? "inc" : "dec");
       inst = dyn_cast<llvm::Instruction>(value);
       if(inst){
          inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
       }
...
Value *ScalarExprEmitter::VisitOffsetOfExpr(OffsetOfExpr *E) {
  ...
  for (unsigned i = 0; i != n; ++i) {
    ...
    case OffsetOfExpr::OffsetOfNode::Array: {
      ...
      inst = dyn_cast<llvm::Instruction>(Offset);
      if(inst){
         if(IdxSigned) 
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
         else
    ...
    inst = dyn_cast<llvm::Instruction>(Result);
      if(inst){
...
void ScalarExprEmitter::EmitUndefinedBehaviorIntegerDivAndRemCheck(
  ...
  if (CGF.SanOpts->SignedIntegerOverflow &&
    ...
    inst = dyn_cast<llvm::Instruction>(Cond);
    if(inst){
       inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
    }
...
Value *ScalarExprEmitter::EmitDiv(const BinOpInfo &Ops) {
  ...
  if (Ops.LHS->getType()->isFPOrFPVectorTy()) {
    ...
    inst = dyn_cast<llvm::Instruction>(Val);
    if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
    }
    return Val;
    ...
    value = Builder.CreateUDiv(Ops.LHS, Ops.RHS, "div"); 
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
    }
    ...
    value = Builder.CreateSDiv(Ops.LHS, Ops.RHS, "div"); 
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
    }
...
Value *ScalarExprEmitter::EmitRem(const BinOpInfo &Ops) {
  ...
  if (Ops.Ty->hasUnsignedIntegerRepresentation()){
    value = Builder.CreateURem(Ops.LHS, Ops.RHS, "rem"); 
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
    }
    ...
    value = Builder.CreateSRem(Ops.LHS, Ops.RHS, "rem"); 
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){ 
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
    }
...
static Value *emitPointerArithmetic(CodeGenFunction &CGF,
  ...
  if (!pointerType) {
    ...
    inst = dyn_cast<llvm::Instruction>(index);
    if(inst){
       if(indexOperand->getType()->isSignedIntegerOrEnumerationType()) 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
       else
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
    }
...
static Value* buildFMulAdd(llvm::BinaryOperator *MulOp, Value *Addend,
  ...
  llvm::Instruction* inst;
  llvm::LLVMContext &VMContext = const_cast<CodeGenFunction&>(CGF).getLLVMContext();
  ...
  if (negMul) {
    ...
    inst = dyn_cast<llvm::Instruction>(MulOp0);
    if(inst){
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
    }
  } else if (negAdd) {
    Addend =
      Builder.CreateFSub(
        ...
        "neg");
    inst = dyn_cast<llvm::Instruction>(Addend);
    if(inst){
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
    }
  }
...
Value *ScalarExprEmitter::EmitAdd(const BinOpInfo &op) {
  ...
  if (op.LHS->getType()->isPointerTy() ||
     ...
     value = emitPointerArithmetic(CGF, op, /*subtraction*/ false);
     inst = dyn_cast<llvm::Instruction>(value);
     if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "pointerAdd")));
     }
     return value; 
  }
  ...
  if (op.Ty->isSignedIntegerOrEnumerationType()) {
    switch (CGF.getLangOpts().getSignedOverflowBehavior()) {
    case LangOptions::SOB_Defined:
      value = Builder.CreateAdd(op.LHS, op.RHS, "add");
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
          inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
      }
    ...
    case LangOptions::SOB_Undefined:
      if (!CGF.SanOpts->SignedIntegerOverflow){
        value = Builder.CreateNSWAdd(op.LHS, op.RHS, "add");
        inst = dyn_cast<llvm::Instruction>(value);
        if(inst){ 
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
        }
      ...
      // Fall through.
    case LangOptions::SOB_Trapping:
       value = EmitOverflowCheckedBinOp(op); 
       inst = dyn_cast<llvm::Instruction>(value);
       if(inst){ 
           inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
        }
  ...
  if (op.LHS->getType()->isFPOrFPVectorTy()) {
    ...
    if (Value *FMulAdd = tryEmitFMulAdd(op, CGF, Builder)){
       inst = dyn_cast<llvm::Instruction>(FMulAdd);
       if(inst){ 
           inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
       }
       return FMulAdd; 
    ...
    value = Builder.CreateFAdd(op.LHS, op.RHS, "add");
    inst = dyn_cast<llvm::Instruction>(value);
    if(inst){ 
      inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
    }
...
Value *ScalarExprEmitter::EmitSub(const BinOpInfo &op) {
  ...
  if (!op.LHS->getType()->isPointerTy()) {
    if (op.Ty->isSignedIntegerOrEnumerationType()) {
      switch (CGF.getLangOpts().getSignedOverflowBehavior()) {
      case LangOptions::SOB_Defined:
        value = Builder.CreateSub(op.LHS, op.RHS, "sub");
        inst = dyn_cast<llvm::Instruction>(value);
        if(inst){ 
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
        }
      ...
      case LangOptions::SOB_Undefined:
        if (!CGF.SanOpts->SignedIntegerOverflow){ 
          value = Builder.CreateNSWSub(op.LHS, op.RHS, "sub");
          inst = dyn_cast<llvm::Instruction>(value);
          if(inst){ 
              inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
          }
    ...
    if (op.LHS->getType()->isFPOrFPVectorTy()) {
      ...
      value = Builder.CreateFSub(op.LHS, op.RHS, "sub"); 
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "float")));
      } 
  ...
  if (!op.RHS->getType()->isPointerTy()){
     value = emitPointerArithmetic(CGF, op, /*subtraction*/ true); 
     inst = dyn_cast<llvm::Instruction>(value);
     if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "pointerSub")));
     }
     return value;
  }
  ...
  inst = dyn_cast<llvm::Instruction>(diffInChars);
  if(inst){ 
      inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "pointerSub")));
  }
  ...
  value = Builder.CreateExactSDiv(diffInChars, divisor, "sub.ptr.div");
  inst = dyn_cast<llvm::Instruction>(value);
  if(inst){ 
      inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signedPointerDiv")));
  }
  return value;
}
...
Value *ScalarExprEmitter::EmitShl(const BinOpInfo &Ops) {
  ...
  if (CGF.SanOpts->Shift && !CGF.getLangOpts().OpenCL &&
    ...
    if (Ops.Ty->hasSignedIntegerRepresentation()) {
      ...
      value = Builder.CreateSub(WidthMinusOne, RHS, "shl.zeros", /*NUW*/true, /*NSW*/true);
      inst = dyn_cast<llvm::Instruction>(value);
      if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
      }
      llvm::Value *BitsShiftedOff = Builder.CreateLShr(Ops.LHS, value, "shl.check"); 
      inst = dyn_cast<llvm::Instruction>(BitsShiftedOff);
      if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
      }
  ...
  if (CGF.getLangOpts().OpenCL){
    ...
    inst = dyn_cast<llvm::Instruction>(RHS);
    if(inst){
        inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned"))); 
    }
  }
  if(Ops.Ty->isUnsignedIntegerType()){ 
     value = Builder.CreateShl(Ops.LHS, RHS, "shl");
     inst = dyn_cast<llvm::Instruction>(value);
     if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
     }
...
Value *ScalarExprEmitter::EmitShr(const BinOpInfo &Ops) {
  ...
  if (Ops.Ty->hasUnsignedIntegerRepresentation()){
     value = Builder.CreateLShr(Ops.LHS, RHS, "shr");
     inst = dyn_cast<llvm::Instruction>(value);
     if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned")));
     }
     ...
     value = Builder.CreateAShr(Ops.LHS, RHS, "shr");
     inst = dyn_cast<llvm::Instruction>(value);
     if(inst){ 
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
...
Value *ScalarExprEmitter::EmitCompare(const BinaryOperator *E,unsigned UICmpOpc,
  ...
  if (const MemberPointerType *MPT = LHSTy->getAs<MemberPointerType>()) {
    ...
    if (E->getOpcode() == BO_EQ) {
      ...
      inst = dyn_cast<llvm::Instruction>(Result);
      if(inst){
         if(E->getType()->hasSignedIntegerRepresentation())
            inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
         else
             inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "unsigned"))); 
      }
...
Value *ScalarExprEmitter::VisitBinLAnd(const BinaryOperator *E) {
  ...
  if (E->getType()->isVectorType()) {
    ...
    inst = dyn_cast<llvm::Instruction>(And);
    if(inst){
	if(E->getType()->hasSignedIntegerRepresentation())
        	inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed"))); 
	else
...
Value *ScalarExprEmitter::VisitBinLOr(const BinaryOperator *E) {
  ...
  if (E->getType()->isVectorType()) {
    ...
    inst = dyn_cast<llvm::Instruction>(Or);
    if(inst){
       if(E->getType()->hasSignedIntegerRepresentation())
          inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed")));
       else
...
VisitAbstractConditionalOperator(const AbstractConditionalOperator *E) {
  ...
  if (CGF.getLangOpts().OpenCL
    ...
    inst = dyn_cast<llvm::Instruction>(tmp3);
      if(inst){
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed"))); 
    }
    inst = dyn_cast<llvm::Instruction>(tmp4);
      if(inst){
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed"))); 
    }
    inst = dyn_cast<llvm::Instruction>(tmp5);
      if(inst){
         inst->setMetadata("opSignedness", llvm::MDNode::get(VMContext, llvm::MDString::get(VMContext, "signed"))); 
    }
    if (wasCast)
