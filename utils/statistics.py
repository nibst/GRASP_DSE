...
def arithmetic_mean_adrs(reference_set, timestamps, comparer=None, metrics=['resources', 'time_latency']):
    if not comparer:
...
def arithmetic_mean_percentage(solutions1, solutions2, comparer=None, metrics=['resources', 'time_latency']):
    if not comparer:
